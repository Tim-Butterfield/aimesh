...
- **ACP and MCP** — these servers are driven by an agent, can serve several conversations at once, and have no single launch folder, so nothing is inferred from where the process started. **Each call declares its scope**: its absolute `workspace` (on ACP, the turn's workspace or the session `cwd`) plus any extra absolute `roots`, and that call's resolver is built from exactly those paths — no call's scope is another's. A relative path is refused as a malformed request. Every declared path is judged on what it **canonicalizes to**: a protected path is refused, and so is an **over-broad** one — `/`, your home directory or its parent, a system tree, a shared tree (`/Users/Shared`, `\Users\Public`) or a mount parent (`/mnt`, `/media`, `/Volumes`, `/srv`), with the Windows entries drive-letter agnostic. A call can never waive that. The operator may additionally set a **root ceiling** with `--root <dir>` (repeatable): every declared path must then lie inside it (`scope_outside_root_ceiling`), and a legacy MCP client's `roots/list` answer narrows further. `--allow-broad-root` lets an operator name a broad `--root` ceiling on purpose; it never waives the denylist and never applies to a declared path. `inlineWorkspace` consumes no scope. Every path in a call, including authority document paths, must resolve inside what that call declared.

A resolver with no roots fails **closed**: every path is refused, with the machine reason `scope_no_roots_configured`. "No roots" is never "unrestricted".
...
### 5. Remediation over MCP and ACP: what the write grant does, and does not, protect

`aimesh review mcp` and `aimesh review acp` are driven by an agent, and both can write. Writing is gated twice. First, the operator's **launch grant**: without `--allow-writes` aimesh never changes project content on that server — `review_remediate` with `output: "patch"` (or an ACP `patch` turn) still supplies the complete diff, and the agent applies it with its own tools; `review_doctor` and the ACP `initialize` result report this as `writes: "agent"`, with `diffAvailable: true`. Second, on MCP, every apply must additionally pass `allowWrite: true`, and on both surfaces an apply must name the completed report run it applies (`fromRun`) and the workspace that run reviewed.
...
**So where does the honest enforcement live?** In things a model cannot supply: the operator's **launch grant** (`--allow-writes`), the **named adapters** and optional **root ceiling** in the host configuration, and **scope confinement** applied to every path a call declares. Everything below is host-enforced in code.

**What the double opt-in actually buys.** It is a *coordination* mechanism with the host's permission layer and with the operator's intent. The launch grant means an operator who never asked for writes gets a server that cannot make them; requiring a literal per-call `allowWrite` means a write is never a default the model drifted into, and leaves a record saying so. The per-call half is a **hint**, exactly like the MCP annotations beside it.
...
- **Scope confinement** (§1). Every path a call names — the workspace and every authority document — must lie inside the absolute paths that call declared, inside the operator's `--root` ceiling when one is set, and outside every protected or over-broad location; a legacy MCP client's `roots/list` answer only narrows it. A remediation's `workspace` must be the tree its source run reviewed.
...
- **One write path.** CLI, ACP and MCP remediation all run through the same governed write, so the journal, content pins, cancel/commit gate, per-finding staging, receipt and edit-target checks apply identically on every surface. Cancellation is honored before the write window and at the commit gate; a commit that has been entered runs to completion, so a result never reports both "cancelled" and "wrote something".
- **Identity binding on the reviewed root.** The decision set carries the reviewed directory's durable identity (device+inode on unix; volume serial and file index on Windows, captured eagerly from an open handle because `os.SameFile` loads the index lazily by path and would compare a swapped-in directory against itself) and canonical path, captured when the review ran. If the path later names a different directory — a swapped checkout, a re-pointed mount — the remediation halts. A path string is not a repository, and a substitute whose targeted files happen to hash the same would otherwise pass every content check.
...
- Name only the adapters you are willing to send content to (`--adapter` in the host configuration that starts the MCP/ACP server). Those named adapters are the whole egress set for that server: a call can use no other. The host configuration is edited by the host's agent or by you; aimesh never writes it.
- Where the agent should only ever work inside known trees, set a `--root <dir>` ceiling (repeatable). Name the **narrowest** directory that contains the work; do not reach for `--allow-broad-root` to make a refusal go away, because the refusal is telling you the ceiling grants more than the work needs.
- Leave the MCP/ACP server **without** `--allow-writes` unless you specifically want aimesh to change your files; the diff is still supplied, so the agent (and its host's own permission prompts) applies it.
...
- **Branch tested only** — the platform-specific code path is exercised on *any* platform by substituting the variable that selects it (the `streamAliasing` / `reparseAttr` / `windowsPaths` pattern), so the branch's **logic** is tested. The behaviour of the real filesystem underneath it is **not**.
...
| Over-broad-root refusal (`/`, `~`, system, shared, mount parents) | Holds | Partly branch tested — the Windows arm is entered and asserted not to misclassify an ordinary directory, but the **drive-letter-agnostic positive match is not reachable off Windows** (see caveat 2 below) | `internal/review/surface/acp/roots.go` |
| Case-insensitive path comparison — allowed roots, the boundary picker for an identity-bound read, the reviewed-root identity check, and the commit destination check | Holds (folding is **off**, which is the correct unix behaviour, and that is asserted in each place) | Branch tested only — five substitutable vars (`scope.foldPaths`, `workspace.foldPaths`, `rootfile.foldPaths`, `run.foldPaths`, `acp.windowsPaths`) let the folding branch run on any platform | `meshcore/scope`, `meshcore/workspace`, `internal/review/access/rootfile`, `internal/review/manager/run`, `internal/review/surface/acp` |
