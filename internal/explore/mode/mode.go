// Package mode is exploremesh's mode registry: a mode is a VERSIONED, app-owned contract
...
//
// It is exploremesh app grammar — meshcore never learns modes. It depends on the schema
...
// Mode names (the app-owned vocabulary).
const (
	// Map (the default) is FORMULATION-FREE: explorers answer with claims + reasons +
	...
	// guidance and the emitted schema.
	Map = "map"
	// Synthesize is FORMULATION-FREE like Map: each explorer gives its single best
	...
	// report — never a tally). Formulation-free keeps the collator out of round-1: the explorer
	// prompt AND schema are app-owned, so the collator cannot dilute the task.
	Synthesize = "synthesize"
	// Catalog is FORMULATION-FREE like Map/Synthesize: each explorer ENUMERATES BROADLY
	...
	Catalog = "catalog"
	// Challenge is an ADJUDICATIVE mode: explorers blindly ATTACK a supplied typed
	...
	// policy (Dual + Confirm) — confirmation covers EVERY count-bearing mode, not Shortlist alone.
	Challenge = "challenge"
	// Shortlist is the ADJUDICATIVE mode that ends in a ranking: explorers blindly ENUMERATE candidates, the
	...
	// HOST — never asserted by a model — and rendered as an informed preference under shared framing.
	Shortlist = "shortlist"
	// AICollab is the shipped COMPOSITION: each agent shortlists its
	...
	AICollab = "ai-collab"
	// Compare is a FIXED-SPACE mode: the user DECLARES the option set and the
	...
	// exactly what makes it the counterexample to the emergent-space modes.
	Compare = "compare"
	// Forecast is the other FIXED-SPACE mode and the most deterministic thing exploremesh
...
// DefaultName is the mode used when a task names none (Map is the default). An empty
// task mode resolves to this.
const DefaultName = Map

// Objective labels what the collator does after the explorer fan-out: collate-only for the
...
const (
	...
	// HOST tally into a ranked subset. The collator never ranks.
	...
	// it returns can reach a governance value.
	ObjectiveHostAggregateCollate Objective = "host_aggregate_collate"
)

// ModeSpec is a mode's app-owned behavior contract. It is a value type registered once at
...
type ModeSpec struct {
	...
	// never authors it (the common cause is removed for fixed-schema modes).
	ExplorerSchema func() schema.Schema
	// Objective is the collator's role after the fan-out.
	Objective Objective
	// Collator is the mode's app-owned PLAIN terminal-collation contract: it renders the
	...
	// Canonicalizing is the mode's app-owned CANONICALIZING terminal-collation contract:
	...
	// FixedSpace is the mode's app-owned FIXED-SPACE terminal contract:
	...
	// Canonicalization is the mode's RANKING-GRADE governance policy over the canonicalization step.
	// Its ZERO VALUE is one decoupled canonicalizer and no confirmation round — what the
	...
	// Rounds is the FIXED number of explorer rounds. 0/1 = the single blind round every mode
	...
	// round.MaxRounds.
	Rounds int
	// LaterRound is the mode's app-owned contract for rounds 2..Rounds. Required when Rounds > 1;
	// nil otherwise.
	LaterRound LaterRoundContract
	// Ballot is the mode's app-owned BALLOT contract, set ONLY by a mode whose
	...
	// ValidateTask is the mode's app-owned check on the USER TASK, run BEFORE any spend. A mode
	...
	// Class is the mode's SPACE class — it selects the honest DEGRADED terminal artifact when the
...
// CanonicalizationPolicy is a mode's governance policy over the canonicalization step.
...
type CanonicalizationPolicy struct {
	...
	// resolved by a versioned HOST rule into a NEW append-only ledger revision. It covers EVERY
...
// An over-large contract is an error, never a clamp.
...
// LaterRoundContract is a mode's app-owned contract for an explorer round AFTER the blind round 1.
// The host — never the mode — decides HOW the prior artifact is framed (delimited, "treat as data",
...
type LaterRoundContract interface {
	...
	// ExplorerSchema is the round's FIXED, app-owned response schema (never collator-authored).
...
func init() {
	// Map: formulation-free; explorers get the deterministic app-owned prompt + the fixed
	...
	// entity resolution — which is why Map's degraded artifact is the raw attributed envelopes plus a
	...
	// Synthesize: formulation-free like Map — the app owns BOTH the explorer prompt
	// ("your single best COMPLETE answer + rationale + assumptions") and the explorer schema, so the
	// collator authors no round-1 schema. The collator then selects/composes the strongest answer.
	register(ModeSpec{
		Name:            Synthesize,
		FormulationFree: true,
	...
	// Catalog: formulation-free like Map/Synthesize — the app owns BOTH the explorer
	// prompt ("enumerate broadly") and schema (candidates[]), so the collator authors no round-1 schema.
	// The terminal collation is CANONICALIZING (Canonicalizing, not Collator): the pipeline runs the
...
// A non-empty unknown mode yields ok=false — the surfaces reject it with a message
// listing Names().
